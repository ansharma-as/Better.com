...
const Dropdown = ({ label, links, isOpen, onMouseEnter, onMouseLeave }) => {
  return (
    ...
    >
      ...
      {isOpen && (
        <ul className="absolute left-0 mt-0 bg-white shadow-lg border rounded-lg w-48 z-10">
          {links.map((link) => (
            <li key={link.label}>
              <Link
                href={link.href}
...
const Navbar = () => {
  ...
  return (
    <nav className="flex justify-between fixed w-screen items-center p-5 bg-white z-20">
      {/* Left Side - Logo and Links */}
      <div className="flex items-center space-x-12">
        {/* Logo */}
        <div className="text-2xl font-bold text-gray-900 px-2">Better</div>
        {/* Links */}
      ...
      <div className="flex items-center space-x-6">
        ...
        <div className="relative group">
          <div className="border rounded-full p-2 hover:bg-slate-300">
            <img
          ...
          <div className="w-72 h-auto absolute left-1/2 transform -translate-x-1/2 translate-y-2 opacity-0 group-hover:opacity-100 bg-white text-black text-xs rounded-lg px-4 py-5 shadow-lg transition-opacity duration-300">
            ...
            <a
              href="tel:4155238837"
              className="underline ml-1 hover:text-green-700"
            >
              (415) 523 8837
            </a>
          </div>
        ...
        <Link
          href="/signin"
          className="text-black hover:rounded-full hover:bg-slate-300 px-6 py-2 text-sm"
        >
          Sign in
        </Link>
        ...
        <Link
          href="/get-started"
          className="text-white bg-green-700 hover:bg-green-900 rounded-full px-6 py-2 text-sm hidden md:flex"
        >
          Get started
        </Link>
      ...
      {isMobileMenuOpen && (
        <div className="fixed flex-col inset-0 bg-white space-y-0 mt-4 z-50 shadow-lg overflow-auto">
          ...
          {/* Tooltip */}
          <div className="flex w-auto items-center h-auto p-4 bg-gray-100 rounded-full text-center border-2 hover:bg-green-500">
            📞 Call us anytime at
            <a
              href="tel:4155238837"
